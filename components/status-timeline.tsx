...
const ESCALATION_LEVELS: Record<number, { label: string; description: string }> = {
...
export function StatusTimeline({
  ...
  const escalationLevel = typeof rawEscalationLevel === "string" 
  ...
  const getSlaStatus = () => {
    ...
    if (hoursRemaining < 0) {
      return { status: "breached", label: "SLA Breached", color: "destructive" as const }
    } else if (hoursRemaining < 24) {
      return { status: "warning", label: "Due Soon", color: "secondary" as const }
    }
    return { status: "ok", label: `Due ${formatDistanceToNow(due, { addSuffix: true })}`, color: "outline" as const }
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="relative">
          ...
          {displayHistory.map((item, index) => {
            ...
            const changedByName = typeof item.changedBy === 'string' 
              ? item.changedBy 
              : item.changedBy?.name
            return (
              <div key={item.id} className="relative flex items-start gap-3 pb-4">
                <div className={`relative z-10 h-6 w-6 rounded-full ${config.bgColor} flex items-center justify-center`}>
        ...
        {statusHistory.length > 3 && (
          ...
          >
            {isExpanded ? "Show less" : `Show ${statusHistory.length - 3} more updates`}
          </button>
        )}
      </CardContent>
    </Card>
...
export function StatusBadge({ 
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Badge 
        variant="outline" 
        className={`${config.bgColor} ${config.color} border-0 ${size === "sm" ? "text-xs px-1.5 py-0" : ""}`}
      >
        {config.icon}
        <span className="ml-1">{config.label}</span>
      </Badge>
      {isUrgent && (
        <Badge variant="destructive" className={size === "sm" ? "text-xs px-1.5 py-0" : ""}>
          <Flag className="h-3 w-3" />
        </Badge>
      )}
    </div>
