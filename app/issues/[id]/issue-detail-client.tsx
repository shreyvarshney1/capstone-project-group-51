...
export function IssueDetailClient({ issue }: IssueDetailClientProps) {
  ...
  // Get all images
  const allImages = issue.images.length > 0 ? issue.images : (issue.imageUrl ? [issue.imageUrl] : [])
  ...
  const locationString = locationParts.length > 0 ? locationParts.join(", ") : issue.address
        ...
        <Card className="mb-6 overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x border-b">
            <div className="p-4 text-center">
              <div className="text-2xl font-bold text-blue-600">{issue._count.votes}</div>
              <div className="text-xs text-muted-foreground">Votes</div>
            </div>
            <div className="p-4 text-center">
              <div className="text-2xl font-bold text-green-600">{issue._count.comments}</div>
                ...
                {isOfficer && issue.aiClassification && (
                  <div className="mt-4 p-3 bg-muted/50 rounded-lg text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>Confidence: {(issue.aiClassification.confidence * 100).toFixed(0)}%</div>
                      {issue.sentimentScore !== null && (
                        <div>Sentiment: {issue.sentimentScore > 0.3 ? "Positive" : issue.sentimentScore < -0.3 ? "Negative" : "Neutral"}</div>
                      )}
                      {issue.priorityScore !== null && (
                        <div>Priority Score: {issue.priorityScore.toFixed(0)}</div>
                      )}
                    </div>
                  </div>
                )}
            ...
            {isOfficer && (
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  {!issue.isUrgent && (
                    <Button variant="outline" className="w-full justify-start text-orange-600" size="sm">
